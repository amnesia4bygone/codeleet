...
public:
    vector<int> spiralOrder(vector<vector<int> > &matrix) {
        ...
        while(step > 0)
        {
            ...
            if( direction == 0)
            {
                for(int i=loop_offset; i<step; i++)
                {
                    result.push_back(matrix[loop_offset][i]);
			printf("1 %d\n", matrix[loop_offset][i]);
                    cur_col++;
                }
			...
			printf("2 %d\n", matrix[i][col_num - loop_offset-1]);
                    cur_row++;
                }
  		...
  		// bug here
			...
			printf("3 %d\n", matrix[row_num - loop_offset-1][i]);
                    cur_col--;
                }
            ...
            {
                for(int i=row_num - loop_offset; i>loop_offset; i-- )
                {
                    result.push_back(matrix[i][loop_offset]);
			printf("4 %d\n", matrix[i][loop_offset]);
                    cur_row--;
                }                
                direction = 0;
                loop_offset++;
